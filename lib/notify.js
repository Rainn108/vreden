  ...
  try {
    ...
    for (let member of participants) {
      ...
      if (chats.action == 'add' && (global.db.data.chats[chats.id]?.welcome || setting.general.autoWelcome)) {
        ...
        var add = `https://editor.vreden.web.id/notify2?background=https://telegra.ph/file/588090b81788803c48717.jpg&title=Welcome&desk=semoga%20betah%20kak!!&avatar=${encodeURIComponent(pp_user)}`;
      ...
      await vreden.sendMessage(chats.id, {
        ...
        var leftImage = `https://editor.vreden.my.id/api/notify?background=https://telegra.ph/file/a9fd3df1d53b8b0f32068.jpg&title=Goodbye&desk=jangan%20balik%20lagi%20kak!!&avatar=${encodeURIComponent(pp_user)}`;
        if (setting.bots.welcomeType == "v1") {
        let button = [{
          "name": "quick_reply",
