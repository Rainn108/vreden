...
exports.smsg = (vreden, m, store) => {
	...
	if (m.key) {
		m.id = m.key.id
		m.isBaileys = (m.id.endsWith("WBSF")) || (m.id.startsWith('AKIRA')) || (m.id.startsWith("VRDN")) || (m.id.length == 18) || (m.id.startsWith('B1EY') && m.id.length === 20) || (m.id.startsWith('BAE5') && m.id.length === 16) || (m.id.startsWith('3EB0') && (m.id.length === 22 || m.id.length === 40))
		m.typeBaileys = (m.id.endsWith("WBSF")) ? "@wibusoft/baileys" : (m.id.startsWith('AKIRA')) ? "github:LT-SYAII/Bail" : (m.id.startsWith("VRDN")) ? "@vreden/baileys" : (m.id.length == 18) ? "www.facebookblueprint.com - CloudAPI" : (m.id.startsWith('B1EY') && m.id.length === 20) ? "github:@nstar-y/Bail" : (m.id.startsWith('BAE5') && m.id.length === 16) ? "@whiskeysockets/baileys@^6.6.0" : (m.id.startsWith('3EB0') && (m.id.length === 22 || m.id.length === 40)) ? "@whiskeysockets/baileys" : "no libary"
		m.userDevice = m.isBaileys ? "web" : getDevice(m.id)
	...
	if (m.message) {
		...
		m.errorReport = async (error, command) => {
			...
			return vreden.sendText(setting.owner.nomor, `*[ ⚠️ ]* ERROR COKK
...
`, m)
		...
		if (m.quoted) {
			...
			m.quoted.isBaileys = m.quoted.id ? (m.quoted.id.endsWith("WBSF")) || (m.quoted.id.startsWith('AKIRA')) || (m.quoted.id.startsWith("VRDN")) || (m.quoted.id.length == 18) || (m.quoted.id.startsWith('B1EY') && m.quoted.id.length === 20) || (m.quoted.id.startsWith('BAE5') && m.quoted.id.length === 16) || (m.quoted.id.startsWith('3EB0') && (m.quoted.id.length === 22 || m.quoted.id.length === 40)) : false
			m.quoted.typeBaileys = m.quoted.id ? (m.quoted.id.endsWith("WBSF")) ? "@wibusoft/baileys" : (m.quoted.id.startsWith('AKIRA')) ? "github:LT-SYAII/Bail" : (m.quoted.id.startsWith("VRDN")) ? "@vreden/baileys" : (m.quoted.id.length == 18) ? "www.facebookblueprint.com - CloudAPI" : (m.quoted.id.startsWith('B1EY') && m.quoted.id.length === 20) ? "github:@nstar-y/Bail" : (m.quoted.id.startsWith('BAE5') && m.quoted.id.length === 16) ? "@whiskeysockets/baileys@^6.6.0" : (m.quoted.id.startsWith('3EB0') && (m.quoted.id.length === 22 || m.quoted.id.length === 40)) ? "@whiskeysockets/baileys" : "no libary" : null
			m.quoted.userDevice = m.quoted.id ? m.quoted.isBaileys ? "web" : getDevice(m.quoted.id) : null
